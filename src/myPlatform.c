    ...
    for (FPos = Format; *FPos != '\0'; FPos++)
    {
        if (*FPos == '%')
        {
            ...
            case 'd':
            {
                ...
                sprintf((char *)&b, "%d", ui);
                sprint((char *)&b); // Ssend("VPF for %d\n");
                break;
            }
            case 'c':
            {
                int ii = va_arg(Args, int);
                sprintf((char *)&b, "%c", ii);
                sprint((char *)&b); // Ssend("VPF for %c\n");
                break;
            }
            case 't':
            {
                struct ValueType *vt = (struct ValueType *)va_arg(Args, struct ValueType *);
                PrintType(vt); // Ssend("VPF for %t\n");
                break;
            }
#ifndef NO_FP
            case 'f':
            {
                ...
                sprintf((char *)&b, "%f", f);
                sprint((char *)&b); // Ssend("VPF for %f\n");
                break;
            }
#endif
            case '%':
            {
                sprint("%"); // Ssend("VPF for %%\n");
                break;
            }
            case '\0':
                FPos--;
        ...
        {
            sprintf((char *)&b, "%c", *FPos);
            b[1] = 0;
            sprint((char *)&b); // Ssend((char *)&b);
        }
    }
}
