...
void CurLinePrint(struct ParseState *Parser)
{
  ...
  while (ps != 0)
  {
    if (i == line)
    {
      *ps = 0;
      ...
      if (pos + 3 >= strlen(cp) && strlen(cp) >= 3)
      {
        line++;
        pos = 0;
      } // if pos is at EOL then we are really going to execute the next program line
    }
    if (lastline == line && lastpos == pos)
    {
      line++;
      pos = 0;
    }
  ...
  if (nwatches > 0)
  {
    ...
    for (int i = 0; i < nwatches; i++)
    {
      ...
      char asIntPointer = 0;
      char asAddress = 0;
      if (strncmp(Ident, "(char *)", 8) == 0)
      {
        Ident += 8;
        asCharPointer = 1;
      }
      if (strncmp(Ident, "(int *)", 7) == 0)
      {
        Ident += 7;
        asIntPointer = 1;
      }
      ...
      // if (VariableDefined((char *)&cln[1]))
      {
        if (TopStackFrame != NULL)
          rslt = TableGet(&TopStackFrame->LocalTable, mycp, &VariableValue, NULL, NULL, NULL);
        if (rslt == 0)
          rslt = TableGet(&GlobalTable, mycp, &VariableValue, NULL, NULL, NULL);
      }
      if (rslt != 0)
      {
        if (i > 0)
      ...
      else
      {
        if (i > 0)
          sendContent(" | ");
        snprintf((char *)&fbuf, 432, "%s not defined", (char *)&watches[i][0]);
        sendContent((char *)&fbuf);
      }
    }
    sendContent("</td></tr>");
    toConsole = SavetoConsole;
  }
  else
    sendContent("</td></tr>");
  toConsole = SavetoConsole;
...
{
  // IncludeRegister("picoc_unix.h", &UnixSetupFunc, &UnixFunctions[0], NULL);             //刘永相编著，有缺失
}
