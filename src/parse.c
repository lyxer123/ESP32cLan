...
enum ParseResult ParseStatement(struct ParseState *Parser, int CheckTrailingSemicolon)
{
   ...
   // DebugPrintln("Done CheckTrailing Semicolon");
    /* if we're debugging, check for a breakpoint */
    //CurLinePrint(Parser);
    if (Parser->DebugMode && Parser->Mode == RunModeRun)
        DebugCheckStatement(Parser);
    //DebugPrintln("Done DebugCheckStatement");
  
    //if (*(Parser->Pos)==0) return ParseResultEOF;
    /* take note of where we are and then grab a token to see what statement we have */   
    ParserCopy(&PreState, Parser);
    
    //DebugPrintln("Done ParserCopy");
    Token = LexGetToken(Parser, &LexerValue, TRUE);
    if (mybrk==1) DebugPrintln("Doing DebugStepParser");    
    if (mybrk==1) 
    {
      DebugStep(Parser);
    }
    //DebugPrintln("Done DebugStepParser");    
   // DebugPrintln("In case token ");
    switch (Token)
    {
        case TokenEOF:
            { return ParseResultEOF;}
            
   // DebugPrintln("Token EOF ");
        case TokenIdentifier:
   // DebugPrintln("Token Identifier ");
    ...
    //DebugPrintln("Token default ");
        ...
        case TokenBreak:
    //DebugPrintln("Token break ");
            if (Parser->Mode == RunModeRun)
                Parser->Mode = RunModeBreak;
            break;
            
        case TokenContinue:
    //DebugPrintln("Token continue ");
            if (Parser->Mode == RunModeRun)
                Parser->Mode = RunModeContinue;
            break;
            
        case TokenReturn:
    //DebugPrintln("Token return ");
            if (Parser->Mode == RunModeRun)
            {
        ...
        case TokenTypedef:
    //DebugPrintln("Token typedef ");
            ParseTypedef(Parser);
            break;
            
        case TokenGoto:
    //DebugPrintln("Token goto ");
